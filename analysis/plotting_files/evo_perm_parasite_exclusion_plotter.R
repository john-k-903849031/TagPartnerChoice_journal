...
perm_color_map <- c("x <= 0" = "#440154FF",
                    ...
                    "0.75 < x <= 1" = "#7AD151FF",             
                    "x > 1" = "#FDE725FF") 
behavior_color_map <- c("Exclusive parasitism"="#891901",
...
subset_combo_df <- combo_df %>%
  filter((param_name == "vt" & param < 0.3) | 
         (param_name == "tag_mut" & param > 0.001)) %>%
  mutate(evo_mut = as.numeric(as.character(evo_mut)),
         category = case_when(evo_mut <= 0.0001 ~ "lower", 
                              evo_mut > 0.0001 ~ "higher",
                              .default = "other"),
         param_label = case_when(param_name == "vt" ~ "VT rate < 30%",
                                 param_name == "tag_mut" ~ "TM rate > 0.1%",
                                 .default="other")) %>%
...
View(
  subset_combo_df %>% 
    ...
    mutate(percent = signif(behavior_count/category_count, digits = 2)*100)
)

View(
  subset_combo_df %>% 
    filter(category=="higher" & behavior_class == "Symbiont extinction")%>%
    ...
    mutate(percent = signif(perm_count/category_count, digits = 2)*100)
)
...
b <- subset_combo_df %>% 
  mutate(facet_label = case_when((category == "lower" & behavior_class == "Exclusive parasitism")~"Lower\npermissiveness mutation\nand exclusive parasitism",
                                 (category == "higher" & behavior_class == "Symbiont extinction")~"Higher\npermissiveness mutation\nand symbiont extinction",
                                 .default = "other"),
         facet_label = factor(facet_label, levels = c("Lower\npermissiveness mutation\nand exclusive parasitism", "Higher\npermissiveness mutation\nand symbiont extinction", "other"))) %>%
