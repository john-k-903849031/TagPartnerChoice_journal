...
fix <- read_csv("../../data/exp_3_base_vt_mut_fixed/org_dump.dat") %>% rename(param = vt)  
combo <- transform_data(evo, fix)
rm(evo, fix)
calculate_p_vals_and_eff_sizes(combo, param_prefix = "_vt", "vt_mut_evo_v_fix_deltas_all.csv")
rm(combo)

### exp 2
#print("** TM **")
#evo <- read_csv("../../data/exp_2_base_tagmut_evolving/org_dump.dat") %>% rename(param = tag_mut)  %>% filter(param != 0)
#fix <- read_csv("../../data/exp_4_base_tagmut_mut_fixed/org_dump.dat") %>% rename(param = tag_mut) %>% filter(param != 0)
#combo <- transform_data(evo, fix)
#rm(evo, fix)
#calculate_p_vals_and_eff_sizes(combo, param_prefix = "_tm", "tm_mut_evo_v_fix_deltas.csv")
#rm(combo)
