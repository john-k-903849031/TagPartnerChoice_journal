...
header_prefix = "seed,tag_perm,vt,"
    ...
    for tp in map(str, t_1): 
        for b in range(0,30):
            for vt in map(str, t_2):
                s = str(seed)
                seed += 1
                in_file_name = in_file_prefix +"_TP"+tp+"_VT"+vt+"_data_SEED" + s + ".data"
                line_prefix = s + "," + tp + "," + vt + ","
                try :
...
def munge_file(out_file_name, out_file_header, in_file_prefix) :
    ...
    loop(100001, treatments_1, treatments_2, '', in_file_prefix, out_file)
    seed = 109001
    for tp in ["none"]:
        for vt in map(str, treatments_2):
            for b in range(0,30):
                s = str(seed)
                seed += 1
                in_file_name = in_file_prefix +"_TP"+tp+"_VT"+vt+"_data_SEED" + s + ".data"
