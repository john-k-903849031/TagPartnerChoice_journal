...
treatments_1 = [0.125, 0.1875, 0.25, 0.3125, 0.375, 0.4375, 0.5, 0.5625, 0.625]
header_prefix = "seed,tag_perm,tag_mut,"

def loop(seed, t_1, t_2, t_3, in_file_prefix, out_file):
    for tp in map(str, t_1): 
        for b in range(0,30):
            for tm in map(str, t_2):
                s = str(seed)
                seed += 1
                in_file_name = in_file_prefix +"_TP"+tp+"_TM"+tm+"_data_SEED" + s + ".data"
                line_prefix = s + "," + tp + "," + tm + ","
                try :
...
def munge_file(out_file_name, out_file_header, in_file_prefix) :
    ...
    loop(80001, treatments_1, treatments_2, '', in_file_prefix, out_file)
    loop(89001, ["none"], ["none"], '', in_file_prefix, out_file)
    out_file.close()
