treatments_1 = [1]
treatments_2 = [0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0]
header_prefix = "seed,evo_mut,tag_mut,"

def loop(seed, t_1, t_2, t_3, in_file_prefix, out_file):
    for metric in map(str, t_1): 
        for b in range(0,30):
            for perm_mut in map(str, t_2):
                s = str(seed)
                seed += 1
                in_file_name = in_file_prefix +"_TM"+t_3+"_PM"+perm_mut+"_data_SEED" + s + ".data"
                line_prefix = s + "," + perm_mut + "," + t_3 + ","
                try :
